...
public class HighResCsvExtractor {
    ...
    public void extract(final long satelliteId) {
        ...
        final Timestamp latestSatelliteTime = highResolutionDao.getLatestSatelliteTime(satelliteId);

        final Timestamp since = new Timestamp(latestSatelliteTime.getTime() - (60 * 60 * 1000));

        final List<HighResolutionEntity> highResOneHour = highResolutionDao.getSinceSatelliteTime(satelliteId, since);

        if (highResOneHour.size() == 0) {
            return;
        }

        final File fileLocation = new File(System.getProperty("csv.hires"));
