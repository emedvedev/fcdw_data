...
public class RealTimeServiceRestImpl extends AbstractServiceRestImpl implements RealTimeServiceRest {
    ...
    public String get(@RequestParam(value = "callback") String callback) {
        ...
        List<RealTimeEntity> entities = realTimeDao.findBySatelliteIdAndSequenceNumberAndFrameType(2L,
                maxSequenceNumber, maxFrameType);
    ...
    public String get(@PathVariable Long satelliteId, @RequestParam(value = "callback") String callback) {
        ...
        List<RealTimeEntity> entities = realTimeDao.findBySatelliteIdAndSequenceNumberAndFrameType(satelliteId,
                maxSequenceNumber, maxFrameType);
        ...
        map.put("data",
                new RealTimeSummary(
                        ...
                        new AsibSummary(
                                String.format(SOL_TEMPERATURE_FORMAT,
                                        scaleAndOffset(realTimeEntity.getC20(), -0.2073, 158.239)),
                                String.format(SOL_TEMPERATURE_FORMAT,
                                        scaleAndOffset(realTimeEntity.getC21(), -0.2083, 159.227)),
                                String.format(SOL_TEMPERATURE_FORMAT,
                                        scaleAndOffset(realTimeEntity.getC22(), -0.2076, 158.656)),
                                String.format(SOL_TEMPERATURE_FORMAT,
                                        scaleAndOffset(realTimeEntity.getC23(), -0.2087, 159.045)),
                        ...
                        new RfSummary(
                                String.format(SOL_TEMPERATURE_FORMAT,
                                        scaleAndOffset(realTimeEntity.getC34(), -0.857, 193.672)),
...
/*
 * EPS: Total Photo Current 0 mA 184 212 Battery Voltage 8226 mV 8286 8296 Total System Current 144
 * mA 190 208 Battery Temp 25 °C 25 25 ASIB: Solar Panel Temp X+ Solar Panel Temp X- Solar Panel
 * Temp Y+ Solar Panel Temp Y- 3.3 Bus Voltage 3.3 Bus Current 5.0 Bus voltage RF: Temperature
 * Receive Current Transmit Current 3.3V bus Transmit Current 5.0V bus PA: Forward Power Reverse
 * Power Device Temperature Bus Current AntS: Antenna Temp 0 Antenna Temp 1 Antenna Deployment 0
 * Antenna Deployment 1 Antenna Deployment 2 Antenna Deployment 3 Software: Sequence Number Data
 * Valid ASIB Data Valid EPS Data Valid PA Data Valid RF Data Valid MSE Data Valid ANTS Bus-B Data
 * Valid ANTS Bus-A In Eclipse Mode In Safe Mode
 */
